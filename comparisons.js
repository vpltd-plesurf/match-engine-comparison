...
window.COMPARISONS_DATA = [
  {
    ...
    "assessmentHistory": [
      ...
      },
      {
        ...
        "gameFeature": 84,
        "fullGame": 91,
        "note": "14 commits, 91 files (37 JS/CS). ME: ball physics retuned (drag+60%, friction+150%, first touch failure, outfield blocking), 6 tackle variants, GK reaction delay+freeze fix+12-type dive, linesman error, referee blind spot, 30+ player states, auto-dispossession. Client: complete replay viewer (intro/scoreboard/celebrations/highlights/1-15x), 16 rarity booster images, squad mgmt polish (gradients/icons/columns), upgrade guards. 8 ME entries resolved. Open ME: 2 (offside, referee). GF +2%."
      }
    ],
    "lastUpdated": "26/02/26 : 14:36"
  },
  {
  ...
  },
  {
    ...
    "fm2026Details": "All 7 original areas addressed PLUS further tuning: (1) First touch failure system (multi-factor: distance, speed, touch, pressure, aggression). (2) Outfield shot blocking (bravery+positioning within 1.5m, chance = bravery/210 + positioning/210 × proximity). (3) Auto-dispossession (6% base within 2m, 25% cap, 0.15s cooldown). (4) Pass interception (0.25 + interception/200 within 2.5m of path). (5) Ball claim vacuum (0.6m outfield, 0.8m GK, 8.0 m/s pull). (6) Ground friction +150% (0.10→0.25). (7) BASE_SPEED 3.2→2.6. Additional: team-wide 70s shot cooldown, chip shot power reduction (0.6x). Scoring should be substantially more realistic. Verification testing still needed.",
    "fm2026Files": [
      "playerStateController.js (computeDecisionInterval — intelligence-based 0.2-4.2s — FIXED)",
      "playerAIController.js (control phase 0.8-2.0s — FIXED)",
      "playerAIController.js (hesitation 8-60% — FIXED)",
      "aiConstants.js (shot thresholds 0.92/0.97 — FIXED)",
      "aiConstants.js (SHOT_INACCURACY_MULTIPLIER=16.0 — FIXED)",
      "playerAIController.js (shot refractory 4s/8s + parry suppression 2s — FIXED)",
      "challengeController.js (challenge trigger 4.5m — FIXED)",
      "physicsConstants.js (GK radii: catch 4.5, parry 6.5, dive 7.5 — FIXED)",
      "physicsConstants.js (air drag 0.024, ground decel 8.5, control lock 1.5s — FIXED)",
      "matchFlowController.js (dead-ball times: FK/Corner 12s, TI/GK/KO 6s — FIXED)",
      "supportController.js (strict defensive shape, goal-threat intercept — FIXED)",
      "aiKeeper.js (reaction 100ms, aerial handling, 6 dive directions — FIXED)"
    ],
    ...
    "gapAnalysis": "[SUBSTANTIALLY RESOLVED 24 Feb] All 7 areas of the comprehensive rebalance have been implemented in 22 commits (18-24 Feb). Decision cadence is now intelligence-based (0.2-4.2s, was 0.15s flat). Ball control time is 0.8-2.0s (was 0.2-0.8s). Shot difficulty dramatically increased (thresholds 0.92/0.97, inaccuracy 16x, refractory memory). Defensive AI overhauled (goal-threat intercept, predictive line, strict shape, proximity-based tackles). GK effectiveness improved (expanded radii, realistic reaction time, 6 directional dives). Dead ball time realistic (6-12s per restart type, celebration 8-12s, walk-to-position).\n\nREMAINING ADVISORY: Verification testing with 20+ matches is needed to confirm the combined effect produces realistic statistics (target: 12-14 shots/team, 1.3-1.5 goals/team, 33-38% on-target rate). Individual area tuning may still be needed — the changes address all 7 areas but the exact parameter values may over- or under-correct. The codeSuggestions below are retained as reference for the original analysis and suggested values.",
