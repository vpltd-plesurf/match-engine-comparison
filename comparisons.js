window.COMPARISONS_DATA = [
  ...
  },
  {
    ...
    "fm2026Details": "[SUBSTANTIALLY FIXED 24 Feb] All 7 areas of the cmp-053 rebalancing plan have been implemented across 22 commits (18-24 Feb).\n\nAREA 1 — DECISION CADENCE: computeDecisionInterval() now uses intelligence-based scaling: base = 0.2 + (100 - combinedIntel) * 0.04, giving 0.2s (intel 100) to 4.2s (intel 0). Vision+Intelligence averaged for combinedIntel. Composure variance 0.1-0.7s. Final-third acceleration 0.75x. ACTION_COOLDOWN 0.3s base. getScaledCooldown() per action type scaled by intelligence.\n\nAREA 2 — BALL CONTROL TIME: controlTimeNeeded = max(0.8, 2.0 - control/100). Control=100→1.0s, Control=0→2.0s. Hesitation chance 8% elite to 60% low-skill. Proactive hesitation (drift forward instead of freeze). POSSESSION_TIME_LIMIT 2.5→5.0s. Panic threshold 2.5→5.0s.\n\nAREA 3 — DEFENSIVE EFFECTIVENESS: Challenge trigger distance 3.0→4.5m. Proximity-based ball acquisition (1.8m for tackles, 1.5m for fatigue snaps). getEffectiveAttribute used throughout. Goal-threat intercept: defenders sprint to intercept carrier driving at goal. Predictive defensive line uses ball velocity. Loose ball secondary defense 12m radius in defensive third.\n\nAREA 4 — PRESSING AND MARKING: Pre-press anticipation at 15m when opponent about to pick up ball. Strict defensive shape (OOP defenders bypass organic movement). Engagement override when attacker penetrates deep. Offside trap: defenders push up 4m when ball moves away.\n\nAREA 5 — SHOOTING DIFFICULTY: Thresholds 0.72→0.92 close, 0.82→0.97 distant. SHOT_INACCURACY_MULTIPLIER 4→16. Error range 12-40 (was 8-28). Role multipliers reduced (ST 1.50→1.25, AM 1.30→1.15). Shot refractory: 4s hard suppress 0.1x, 8s zone suppress 0.5x. Parry rebound suppressor: 2s after parry ALL shots 0.2x.\n\nAREA 6 — GOALKEEPER: GK_CATCH_RADIUS 3.0→4.5, GK_PARRY_RADIUS 4.5→6.5, GK_DIVE_RANGE 6.0→7.5. Reaction time 0.03→0.10s (more realistic). Pickup 1.2→2.0m. Ball catch 2.5→3.5m. 6 directional dive states. Aerial catch/punch handling decision. GK aerial advantage 1.5x in box.\n\nAREA 7 — DEAD BALL TIME: FK 12s, Corner 12s, TI/GK/KO 6s. Per-type setup delays (TI 4s, GK 5s, Corner 3s, FK 2s). Celebration 8-12s with walk-to-position (no teleport). Readiness check before restart. Double-touch rule enforcement. Ceremony delays for cards properly observed.\n\nVERIFICATION NEEDED: 20+ test matches should confirm realistic scorelines. Previous calibration targets (Premier League 2024-25): Shots 12-14/team, Goals 1.3-1.5/team, On-target rate 33-38%, GK save rate ~72%.",
    "fm2026Files": [
