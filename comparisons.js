...
window.COMPARISONS_DATA = [
  ...
  {
    "id": "cmp-053-match-time-compression",
    "date": "2026-02-17T12:00:00Z",
    "category": "matchEngine",
    "feature": "Match Time Compression (Simulation Duration)",
    "existsInFM2026": "no",
    "existsInLegacy": "yes",
    "priority": "P0",
    "status": "open",
    "fm2026Details": "FM2026 simulates the FULL 90 minutes in real time: totalMinutes=90 → 5,400 seconds → 108,000 ticks at 20Hz. Every second of a real 90-minute football match is simulated tick-by-tick. Display time equals simulation time (1:1 mapping). Key constants: tickSeconds=1/20 (matchMain.js:35), HALF_TIME=2700 (gameEngine.js:77), FULL_TIME=5400 (gameEngine.js:78). The main loop runs while timeSeconds < (totalMinutes*60) + extraTime (matchMain.js:417). Player speeds are realistic (BASE_SPEED=3.2 m/s, max ~4.8 m/s in movementController.js:16,81). AI decisions occur every 0.5-1.15s (playerStateController.js:147-160). Shot cooldown is 1.5s (playerEngine.js:133). Over 5,400 simulated seconds at these action rates, each team generates ~180-270 shot opportunities, converting to 18-27 goals. This perfectly explains the observed 23-18 and 32-26 scorelines.",
    "fm2026Files": [
      "matchMain.js:35 (tickSeconds: 1/20)",
      "matchMain.js:417 (getRequiredDuration = totalMinutes*60)",
      "gameEngine.js:77-78 (HALF_TIME=2700, FULL_TIME=5400)",
      "matchesRunner.js:69 (totalMinutes: 90)",
      "movementController.js:16 (BASE_SPEED=3.2)",
      "playerStateController.js:147-160 (decision interval 0.5-1.15s)",
      "playerEngine.js:133 (shot cooldown 1.5s)",
      "coreEngine.js:38-41 (timeSeconds += dt)"
    ],
    "legacyDetails": "Legacy compresses the entire 90-minute match into just 600 seconds (10 minutes) of simulation time, then DISPLAYS it as 90 minutes using a TIMESCALE mapping. Key constants: HALF_LENGTH=300 seconds per half (GameManager.cs:24), TIMESCALE=750 game-time-units per simulated second (GameManager.cs:26). Total game time units per match: 600*750 = 450,000. Display conversion: TIMESCALE/9 = 83.33 units per display second, so 450,000/83.33/60 = 90 display minutes. The engine is EVENT-DRIVEN (not fixed tick) — game time jumps directly to the next scheduled event via FindNextReqStamp() (GameManager.cs:2146-2265), skipping empty time. Events include: ball bounces, player AI decisions (BrainStamp), collisions, game state transitions. Player speeds, ball physics, AI cadence, and shot rates are ALL calibrated for this compressed 600-second match. Result: ~20-30 shots per team → 2-3 goals per team (realistic).\n\nThe 9:1 compression ratio is the fundamental reason Legacy produces realistic scorelines while FM2026 produces 20+ goals per side. With 9x less simulation time, there are 9x fewer possessions, 9x fewer shots, and 9x fewer goals — matching real football.",
    "legacyFiles": [
      "GameManager.cs:24 (HALF_LENGTH=300)",
      "GameManager.cs:26 (TIMESCALE=750)",
      "GameManager.cs:928-929 (display time = clock / (TIMESCALE/9))",
      "GameManager.cs:1979-2005 (Clock() event-driven advancement)",
      "GameManager.cs:2146-2265 (FindNextReqStamp() event scheduling)",
      "GameManager.cs:1737-1943 (MatchLoop jumps between events)"
    ],
    "gapAnalysis": "ROOT CAUSE OF SCORING INFLATION. FM2026 simulates 5,400 seconds; Legacy simulates 600 seconds — a 9:1 ratio. All individual scoring fixes applied on 16 Feb (shot thresholds, GK improvements, accuracy, cooldowns, vision gate) are correctly tuned for the action rates per second of gameplay, but they cannot compensate for having 9x more gameplay seconds. The math is clear:\n\n- Legacy: 600s simulation, ~1 shot per 20-30s → 20-30 total shots → 2-3 goals per team\n- FM2026: 5,400s simulation, same shot cadence → 180-270 total shots → 18-27 goals per team\n\nThis is NOT a scoring mechanics bug — it's a fundamental simulation architecture difference. No amount of threshold/cooldown tuning will fix it without either (a) compressing match time or (b) reducing all action rates by 9x (which would make individual moments feel sluggish).\n\nPrevious scoring fixes (cmp-043 through cmp-052) were correct and necessary for per-shot realism, but they address the wrong scale of the problem. The 9x time inflation dwarfs all other factors.",
    "codeSuggestions": "RECOMMENDED: Option B — Simulation Compression Factor\n\nIntroduce a time compression ratio so FM2026 simulates ~600-900 seconds of physics time while displaying 0-90 minutes. This preserves all existing mechanics (which are well-tuned per-second) while producing realistic match totals.\n\n--- STEP 1: Add compression config ---\nFile: matchMain.js (around line 35)\n\nCurrent:\n  tickSeconds: 1 / 20,\n\nAdd to config:\n  tickSeconds: 1 / 20,\n  simulationMinutes: 10,    // actual physics duration (Legacy equivalent)\n  displayMinutes: 90,       // what the clock shows\n  // compressionRatio is derived: 90/10 = 9\n\n--- STEP 2: Change main loop duration ---\nFile: matchMain.js (around line 417)\n\nCurrent:\n  const getRequiredDuration = () => ((config.totalMinutes || 90) * 60) + totalExtraTime + (this.matchState.injuryTime || 0);\n\nChange to:\n  const simMinutes = config.simulationMinutes || 10;\n  const getRequiredDuration = () => (simMinutes * 60) + totalExtraTime + (this.matchState.injuryTime || 0);\n\n--- STEP 3: Add display time mapping ---\nFile: coreEngine.js (around line 38-41)\n\nCurrent:\n  advance(dt) {\n      if (!this.isBound) return;\n      this.matchState.timeSeconds += dt;\n  }\n\nChange to:\n  advance(dt) {\n      if (!this.isBound) return;\n      this.matchState.timeSeconds += dt;  // physics time (0-600s)\n      const ratio = (this.matchState.config?.displayMinutes || 90) / (this.matchState.config?.simulationMinutes || 10);\n      this.matchState.displaySeconds = this.matchState.timeSeconds * ratio;  // display time (0-5400s)\n  }\n\n--- STEP 4: Update half-time / full-time checks ---\nFile: gameEngine.js (around lines 77-78)\n\nCurrent:\n  const HALF_TIME = 2700;\n  const FULL_TIME = 5400;\n\nChange to:\n  const simMinutes = config?.simulationMinutes || 10;\n  const HALF_TIME = (simMinutes * 60) / 2;  // 300 for 10-min sim\n  const FULL_TIME = simMinutes * 60;         // 600 for 10-min sim\n\n--- STEP 5: Update all event timestamps for display ---\nFile: matchFlowController.js and anywhere events are logged\n\nAll match events (goals, cards, subs, etc.) should use displaySeconds for their reported minute:\n  const displayMinute = Math.floor(matchState.displaySeconds / 60);\n\nInstead of:\n  const minute = Math.floor(matchState.timeSeconds / 60);\n\n--- STEP 6: Update late-game awareness ---\nFile: playerAIController.js (around line 594)\n\nCurrent:\n  const totalMinutes = matchState.config?.totalMinutes || 90;\n  const currentMinute = matchState.timeSeconds / 60;\n  const isLate = currentMinute > (totalMinutes * 0.85);\n\nChange to:\n  const displayMinutes = matchState.config?.displayMinutes || 90;\n  const currentDisplayMinute = (matchState.displaySeconds || matchState.timeSeconds) / 60;\n  const isLate = currentDisplayMinute > (displayMinutes * 0.85);\n\n--- STEP 7: Update injury time ---\nFile: matchFlowController.js (around line 433)\n\nInjury time of 6s per goal should be in SIMULATION seconds, not display seconds. Current value of 6s is fine for a 600s match (1% of match). If you want it proportional, keep it as-is.\n\n--- STEP 8: Update matchesRunner default ---\nFile: matchesRunner.js (around line 69)\n\nCurrent:\n  totalMinutes: details.totalMinutes || 90,\n\nChange to:\n  simulationMinutes: details.simulationMinutes || 10,\n  displayMinutes: details.displayMinutes || 90,\n\n--- EXPECTED RESULTS ---\nWith simulationMinutes=10:\n- 600 seconds of physics simulation\n- 12,000 ticks (vs 108,000 before) — 9x faster execution!\n- ~20-30 shots per team → 2-3 goals per team\n- Display shows 0-90 minutes (mapped from 0-600s)\n- All existing per-tick mechanics work unchanged\n- Server processes matches 9x faster (major performance win)\n\n--- TUNING ---\nIf 10 minutes feels too compressed, try simulationMinutes=12 or 15.\n- 12 min → 14,400 ticks, ~3-4 goals per team\n- 15 min → 18,000 ticks, ~4-5 goals per team\nThe sweet spot is likely 10-12 minutes to match Legacy's 2-3 goals per team.\n\n--- ALTERNATIVE (Option A - Quick Test) ---\nFor a quick proof-of-concept, just change totalMinutes from 90 to 10 in matchesRunner.js:69 and run a batch of matches. The display times will show 0-10 min instead of 0-90, but the GOAL COUNTS will immediately become realistic. This confirms the diagnosis before doing the full display-time mapping work."
  }
];
